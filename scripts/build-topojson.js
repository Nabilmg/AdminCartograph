...
// Emit a TS file the visual imports at compile time so the country
// dropdown reflects exactly what the bundle contains.
const generatedDir = path.join(__dirname, "..", "src", "generated");
fs.mkdirSync(generatedDir, { recursive: true });
const bundled = countries
  .filter((c) => summary.find((s) => s.iso3 === c.iso3 && s.status === "ok"))
  .map((c) => ({ iso: c.iso3, name: c.name }));
const tsBody = `// AUTO-GENERATED by scripts/build-topojson.js. Do not edit.
export interface BundledCountry { iso: string; name: string; }
export const BUNDLED_COUNTRIES: BundledCountry[] = ${JSON.stringify(bundled, null, 2)};
`;
fs.writeFileSync(path.join(generatedDir, "countries.ts"), tsBody);
console.log(`Wrote src/generated/countries.ts (${bundled.length} bundled countries)`);
